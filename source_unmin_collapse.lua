...
local aa = {
    function
        ...
        local t, u, v =
            e(s.Notification),
            p.New,
            protectui or protect_ui or protectgui or protect_gui or syn and syn.protect_gui or
        function
            ()
        end
    local w = u("ScreenGui", {})
    ...
    function
        ()
                w.Parent =
                    ...
                    game:GetService "CoreGui":WaitForChild("RobloxGui", math.huge)
    end
,
function
    ()
...
function
    x.SetTheme(C, D)
    if
        x.Window and table.find(x.Themes, D)
        then
        x.Theme = D
                p.UpdateTheme()
    end
...
for
    l, m in pairs(game:GetService "Lighting":GetChildren()) do
                    k(m)
end
if
    game:GetService "Workspace".CurrentCamera
    then
    for
        n, o in pairs(game:GetService "Workspace".CurrentCamera:GetChildren()) do
                        k(o)
    end
end
end
...
local C = e(k.Components.Button)("", s.ButtonHolder, true)
                C.Title.Text = A
for
    D, E in next, s.ButtonHolder:GetChildren() do
    if
        E:IsA "TextButton"
        then
...
function
    ...
    function
        ...
        function
            ...
            if
                not o.Input:IsFocused() or o.Input.TextBounds.X <= q - 2 * p
                then
...
function
    (M)
    if
        M.UserInputType == Enum.UserInputType.MouseButton1 or M.UserInputType == Enum.UserInputType.Touch
        then
        w = true
                        y = M.Position
                        z = v.Root.Position
...
function
    (M)
    if
        A == true or M.UserInputType == Enum.UserInputType.Touch
        then
        A = false
...
function
    v.Minimize(M)
                ...
                local Root = v.Root
                local TabHolder = v.TabHolder
                local ContainerCanvas = v.ContainerCanvas or (v.Window and v.Window.ContainerCanvas)

                if v.Minimized then
                    v._PrevSize = v.Size
                    if TabHolder then TabHolder.Visible = false end
                    if ContainerCanvas then ContainerCanvas.Visible = false end
                    TweenService:Create(Root, info, { Size = UDim2.fromOffset(v.Size.X.Offset, CollapsedHeight) }):Play()
                else
                    local target = v._PrevSize or v.Size
                    TweenService:Create(Root, info, { Size = target }):Play()
                    task.delay(0.18, function()
                        if TabHolder then TabHolder.Visible = true end
                        if ContainerCanvas then ContainerCanvas.Visible = true end
                    end)
                end
...
function
    k.UpdateTheme()
    for
        m, n in next, k.Registry do
        for
            o, p in next, n.Properties do
                    m[o] = k.GetThemeProperty(p)
        end
end
for
    o, p in next, k.TransparencyMotors do
...
function
    ...
    for
        q, r in next, k.DefaultProperties[m] or {} do
                p[q] = r
    end
for
    s, t in next, n or {} do
...
function
    ...
    if
        ...
        then
        while
            h:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
...
function
    ...
    function
        ()
                    local w = 0
        if
            ai.ViewportSize.Y - p.AbsolutePosition.Y < v.AbsoluteSize.Y - 5
            then
...
function
    ...
    if
        ...
        if
            ah.X < B.X or ah.X > B.X + C.X or ah.Y < (B.Y - 20 - 1) or ah.Y > B.Y + C.Y
            then
            l:Close()
        end
end
...
function
    l.BuildDropdownList(B)
                local C, D = l.Values, {}
    for
        E, F in next, t:GetChildren() do
        if
...
function
    ...
    if
        ...
        local U = not N
        if
            l:GetActiveValues() == 1 and not U and not j.AllowNull
            then
        else
        if
            j.Multi
...
elseif
type(j.Default) == "number" and l.Values[j.Default] ~= nil
then
...
function
    (m)
    if
        not i and not af:GetFocusedTextBox() and h.Mode == "Toggle"
        then
        local n = h.Value
        if
            n == "RMB" and m.UserInputType == Enum.UserInputType.MouseButton2
            then
            h.Toggled = not h.Toggled
                            h:DoClick()
        elseif
        m.UserInputType == Enum.UserInputType.Keyboard and m.KeyCode.Name == n
        then
        h.Toggled = not h.Toggled
                            h:DoClick()
    end
...
function
    ...
    function
        (p)
        if
            p.UserInputType == Enum.UserInputType.MouseButton1 or p.UserInputType == Enum.UserInputType.Touch
            then
            i = true
        end
end
...
-horizontal"] = "rbxassetid://10709753692",
                ["lucide-align-
end
-vertical"] = "rbxassetid://10709753808",
                ["lucide-align-horizontal-distribute-center"] = "rbxassetid://10747779791",
                ["lucide-align-horizontal-distribute-
end
"] = "rbxassetid://10747784534",
                ["lucide-align-horizontal-distribute-start"] = "rbxassetid://10709754118",
                ["lucide-align-horizontal-justify-center"] = "rbxassetid://10709754204",
                ["lucide-align-horizontal-justify-
end
"] = "rbxassetid://10709754317",
                ...
                ["lucide-align-vertical-distribute-
end
"] = "rbxassetid://10709760434",
                ["lucide-align-vertical-distribute-start"] = "rbxassetid://10709760612",
                ["lucide-align-vertical-justify-center"] = "rbxassetid://10709760814",
                ["lucide-align-vertical-justify-
end
...
type %s is unsupported"):format(aj, c), 2)
end
function
    ai.new(c, d)
...
function
    ...
    for
        e, f in pairs(d) do
                local g = assert(c._motors[e], ("Unknown motor
        for
            key %s"):format(e))
                g:setGoal(f)
        end
...
function
    ...
    if
        ...
        ag.connected = false
        for
            ah, ai in pairs(ag.signal._connections) do
            if
                ai == ag
                then
...
function
    ag.fire(ah, ...)
    for
        ai, aj in pairs(ah._connections) do
                aj._handler(...)
    end
for
    c, d in pairs(ah._threads) do
                coroutine.resume(d, ...)
end
ah._threads = {}
end
...
for
    ag, ah in next, ab:GetChildren() do
            local aj = ac(ah)
            af[aj.Name] = aj
end
return af
...
for
    ...
    function
        ...
        if
            not x[C]
            then
            ad("Expected ':' not '.' calling member
            function
                " .. A, 1)
            end
        return B(C, ...)
